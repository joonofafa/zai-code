...
## 13. 트레이 상주 + 공유 폴더 → 문서함 동기화

데스크톱만의 강점: **백그라운드 상주**로 공유 폴더 변화를 감시해 조직 문서함에 계속 반영한다.

### 13.1 트레이 상주 모델
- 창을 닫아도 종료되지 않고 **트레이(알림 영역)에 상주**. `ShutdownMode = OnExplicitShutdown`.
- 창 `Closing` → `Cancel + Hide`(트레이로 숨김). 트레이 아이콘 클릭/‘열기’ 메뉴로 복원.
- 트레이 메뉴: **열기 / 동기화 상태 / (설정) / 종료**. ‘종료’로만 진짜 종료.
- Avalonia `TrayIcon` + `NativeMenu` 사용(현재 스택 그대로). 아이콘: `Assets/tray.ico`.
- 데몬/IPC 아님 — 트레이 앱 **자신이** 상주 프로세스이자 임베드된 코어 호스트.

### 13.2 공유 폴더 → 문서함 동기화
```
FolderSyncService (백그라운드)
  연결된 폴더[FileSystemWatcher] → 변경/생성 감지
    → 문서형 필터 → 디바운스
    → 업로드 매니페스트(해시·mtime)로 중복/미변경 스킵
    → OrgDocsUpload(orgId, visibility)   ← 이미 구현된 툴(디렉토리·다중·orgId 자동해소)
    → 트레이 알림("문서 3건이 문서함에 반영됨")
```
- **연결 단위**: `ConnectedFolder(path, orgId, visibility)` — 폴더마다 조직/공개범위 매핑.
- **삭제 전파**: 기본 **안 함**(문서함 실수 삭제 방지). 옵션으로만.
- **재사용**: `OrgDocsUpload`(v1.6.1 디렉토리·다중 업로드), 필요 시 `ChunkBuild`로 로컬 인덱스 병행.
- **뼈대 현황**: `Sync/FolderSyncService.cs` — 감시/이벤트 구조만(현재 `EnableRaisingEvents=false`).
  실제 디바운스·매니페스트·업로드 연결은 다음 단계.

### 13.3 결정 필요(동기화)
1. 자동 동기화 vs 수동("지금 동기화") — 기본 정책.
2. Windows 시작 시 자동 실행(로그인 상주) 여부.
3. 삭제 전파(기본 미전파 권장) 확정.
4. 폴더당 조직/공개범위 지정 UI(설정 화면).
