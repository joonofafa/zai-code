...
- 방식: 정적 코드 리뷰
- 상태: **부분 조치됨 (Critical 4/4 해결, High/Medium 미착수)**

## 조치 이력 (2026-08-02, v1.9.2)

Critical 4건을 모두 해결했다. 상세는 각 항목의 "조치" 메모 참조.

| 항목 | 상태 | 요지 |
|---|---|---|
| SEC-001 프로젝트 MCP 자동 실행 + 키 상속 | ✅ 해결 | 프로젝트 MCP 는 `MOAI_ALLOW_PROJECT_MCP=1` opt-in 전까지 미로드(사용자 홈 MCP 우선), 자식 프로세스에서 민감 환경변수 제거 |
| SEC-002 프로젝트 설정 baseUrl 탈취 | ✅ 해결 | 프로젝트 설정 레이어 `allowSensitive:false` (baseUrl/provider/host/account/proxy/permission 불가) |
| SEC-003 프로젝트 자동 검증 명령 주입 | ✅ 해결 | 프로젝트 설정 레이어 `allowAutomation:false`. (실행 경로의 권한 게이트 통합은 여전히 권고 사항) |
| SEC-004 하위 에이전트 자동 승인 | ✅ 해결 | 서브에이전트가 부모 권한 게이트를 사용(워크스페이스 경계·deny·파괴적 명령 차단·위험 분류 적용) |

High 4건(SEC-005~008)·Medium 2건(SEC-009~010)은 미착수. 아래 원본 리뷰 유지.
